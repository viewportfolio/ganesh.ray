...
function cardMarkup(item, isReel) {
  return `
    <article class="video-card ${isReel ? 'reel-card' : ''}" data-id="${item.id}" data-category="${item.category}" data-title="${item.title.replace(/"/g, '&quot;')}" data-desc="${item.desc.replace(/"/g, '&quot;')}" tabindex="0" role="button" aria-label="Play preview: ${item.title.replace(/"/g, '&quot;')}">
...
const modalExternal = document.getElementById('videoModalExternal');
...
function openVideo(card) {
  ...
  const category = card.getAttribute('data-category') || 'Preview';
  ...
  if (modalExternal) modalExternal.href = `https://drive.google.com/file/d/${id}/view`;
...
function closeVideo() {
  ...
  modalFrame.innerHTML = ''; // Stop video playback immediately
...
// Explicit Close Action (Clicks on video player never propagate to close the modal)
...
const revealTargets = document.querySelectorAll(
  '.section-kicker, .section-title, .section-desc, .video-card, .timeline-item, .achievement-card, .skill-card, .tool-glow-box'
);
